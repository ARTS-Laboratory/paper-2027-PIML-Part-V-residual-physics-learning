...
# from scipy.ndimage import gaussian_filter1d   # Uncomment later if you want to use Gaussian smoothing
"""
Delta learning approach
Step 1: Train model_1 on no friction data for 0-120 s.
Step 2: Create model_2 to predict difference between no friction and with
friction tests. Train this model on full 0-120 s of friction tests.
"""
#%%
"""
The training generator creates a sort-of virtual array so that passing
over the data per epoch is done optimally.
returns batches
"""
...
def main():
    ...
    # downsample by a factor of 20 so that sampling rate is 50 S/s
    ...
    model_1.save('./model_saves/delta_model_1.keras')
    ...
    # Use FULL 0-120 s for training Delta NN
    x_train_wf = x_wf[:80, :]
    v_train_wf = v_wf[:80, :]
    a_train_wf = a_wf[:80, :]
    k_train_wf = k_wf[:80, :]
    ...
    # normalize using the same mean and std from no-friction data
    ...
    # Improved Model 2 with LeakyReLU + L2 regularization
    model_2 = keras.models.Sequential([
        Dense(128, activation='leaky_relu', kernel_regularizer=tf.keras.regularizers.l2(0.001)),
        Dense(128, activation='leaky_relu', kernel_regularizer=tf.keras.regularizers.l2(0.001)),
        Dense(64, activation='leaky_relu', kernel_regularizer=tf.keras.regularizers.l2(0.001)),
    ...
    combined_model.save('./model_saves/delta_combined_model.keras')
    ...
    for i in range(20):
        # model 1
        k_pred = model_1.predict(sliding_window_view(X_test_nf[i], [train_len]), verbose=0)
        k_pred = k_pred * k_std + k_m
        k_nf_model_1[i] = k_pred.flatten()
        
        # combined model
        k_pred = combined_model.predict(sliding_window_view(X_test_nf[i], [train_len]), verbose=0)
        k_pred = k_pred * k_std + k_m
        # k_pred = gaussian_filter1d(k_pred.flatten(), sigma=2.0)   # Uncomment later for Gaussian smoothing
    ...
    for i in range(20):
        # model 1
        k_pred = model_1.predict(sliding_window_view(X_test_wf[i], [train_len]), verbose=0)
        k_pred = k_pred * k_std + k_m
        k_wf_model_1[i] = k_pred.flatten()
        
        # combined model
        k_pred = combined_model.predict(sliding_window_view(X_test_wf[i], [train_len]), verbose=0)
        k_pred = k_pred * k_std + k_m
        # k_pred = gaussian_filter1d(k_pred.flatten(), sigma=2.0)   # Uncomment later for Gaussian smoothing
